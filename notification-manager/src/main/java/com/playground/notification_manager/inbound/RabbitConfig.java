...
public class RabbitConfig {
    ...
    public Queue notificationQueue() {
        var queueName = messagingConfiguration.getNotifications().getQueue();
    ...
    public DirectExchange notificationExchange() {
        var exchangeName = messagingConfiguration.getNotifications().getExchange();
        return ExchangeBuilder.directExchange(exchangeName).durable(true).build();
    }

    @Bean
    public Binding notificationBinding() {
        var bindingKey = messagingConfiguration.getNotifications().getNotificationsBindingKey();
        return BindingBuilder.bind(notificationQueue()).to(notificationExchange()).with(bindingKey);
    }
